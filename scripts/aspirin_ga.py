...
def main():
    ...
    train_data_file = str(data_dir / "aspirin/aspirin_train_data_{}.p".format(args_dict["seed"]))
    val_data_file_num = 1 if args_dict["seed"] == 5 else args_dict["seed"] + 1 
    val_data_file = data_dir / "aspirin/aspirin_train_data_{}.p".format(val_data_file_num)
    ...
    ga.run(population_size=20, num_generations=3, target_groups_pct=0.52, max_order=7, crossover_prob=0.75, 
           mutation_prob=0.2, enable_parallel=True, parallel_workspace=workspace, time_limit="12:00:00", 
    ...
    for i in range(num_eval_trials):
        ...
        curr_test_config["amptorch_config"]["optim"]["epochs"] = 5000
        curr_test_config["amptorch_config"]["cmd"]["seed"] = 1

        test_configs.append(curr_test_config)

        curr_train_data_file = str(data_dir / "aspirin/aspirin_train_data_{}.p".format(i + 1))
        curr_test_data_file = str(data_dir / "aspirin/aspirin_test_data_{}.p".format(i + 1))
